...
from keras.layers import Dense, Dropout, LeakyReLU, BatchNormalization, LSTM, Bidirectional
...
def get_lstm_model(input_shape):
    ...
    # model.add(Bidirectional(LSTM(32, activation='relu', return_sequences=True),input_shape=input_shape ))
...
for j, (train_idx, val_idx) in enumerate(folds):
    ...
    model = get_lstm_model((None, x.shape[2]))
    history = model.fit(X_train_cv, y_train_cv,
                        validation_data=(X_valid_cv, y_valid_cv),
                        # callbacks=[EarlyStopping(patience=20)],
                        epochs=150,
                        verbose=0)
    scores.append(model.evaluate(x, y,verbose=0))
    print(scores[-1])
